...
/**
  * This is an event sourced entity. It has a state, [[LagontestState]], which
  ...
  */
class LagontestEntity extends PersistentEntity {

  override type Command = LagontestCommand[_]
  override type Event = LagontestEvent
  override type State = LagontestState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: LagontestState = LagontestState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case LagontestState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        LagontestState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class LagontestState(message: String, timestamp: String)

object LagontestState {
  /**
    ...
    */
  implicit val format: Format[LagontestState] = Json.format
}

/**
  * This interface defines all the events that the LagontestEntity supports.
  */
sealed trait LagontestEvent

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends LagontestEvent
...
/**
  ...
  */
sealed trait LagontestCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends LagontestCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String, organization: Option[String]) extends LagontestCommand[String]
...
/**
  ...
  * application loader.
  */
object LagontestSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[LagontestState]
  )
}
